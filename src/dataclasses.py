...
class QuestionV1Retrobust:
    ...
    question: str  # original question
    statement: StatementResult  # the parsed statement after calling local model
    decompositions: List[str]  # decompositions from local model
    decompsition_steps: List[List[DecompositionStepV1]]  # reasoning traces
    traces_entailments: List[List[TraceResult]]
    ...
    def __init__(
        ...
        question: str,
        prompt: str,
        model_accessor: LocalModelAccessor,  # Using LocalModelAccessor instead of GptAccessor
        num_decompositions: int = 5,
    ):
        self.question = question
        self.send_question_separately = Config().get(
        ...
        self.model_accessor_indices_with_temperature_0 = Config().get(
            "decomposition.model_accessor_indices_with_temperature_0"
        )
        self.curr_prompt = (
            prompt if self.send_question_separately else prompt + self.question
        )
    ...
    def populate_statement(self):
        local_statement_res = self.model_accessor.call_local_model(self.curr_prompt, "")
        self.statement = format_statement(local_statement_res)

    def populate_decompositions(self):
        def get_temp_at_index(index: int) -> float:
            # check if the index should get temp 0
            if (
                self.model_accessor_indices_with_temperature_0 is not None
                and index in self.model_accessor_indices_with_temperature_0
            ):
                return 0
            return Config().get("decomposition.model_accessor_temperature")

        self.decompositions = [
            self.model_accessor.call_local_model(self.curr_prompt, "", get_temp_at_index(i))
            if not self.send_question_separately
            else self.model_accessor.call_local_model(
                self.curr_prompt, self.question, get_temp_at_index(i)
            )
            for i in range(self.num_decompositions)
        ]
